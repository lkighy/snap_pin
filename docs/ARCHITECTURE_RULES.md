...
Rust Core Service
        ...
        |
        +-- platform_win32: DXGI / WGC / Win32 window APIs
        |
        +-- egui_overlay process or window: overlay and pin rendering
```

Tauri 负责用户入口、配置、历史和托盘，不直接实现截图、OCR、翻译或 Win32 细节。

egui overlay 负责所有对性能和透明窗口行为敏感的视觉交互，包括全屏透明 overlay、鼠标选区、放大镜、标注、贴图窗口、浮动 OCR/翻译文本和实时渲染更新。

core_service 负责调度和状态，不直接依赖 Tauri、egui 或 Win32 UI 类型。

platform_win32 是唯一允许集中接触 Windows 原生 API 的 crate。未来如需 `unsafe`，必须限制在该 crate 内，并用小函数包裹。
...
- `crates/platform_win32`：Windows API 适配层，DXGI/WGC 截屏、透明窗口、穿透、置顶、全局热键、剪贴板。
...
core_service -> ipc, shared_models
platform_win32 -> shared_models
ipc -> shared_models
shared_models -> no internal project crate
```

禁止 `core_service` 依赖 `apps/tauri_desktop` 或 `apps/egui_overlay`。禁止 `shared_models` 依赖任何上层 crate。禁止在 Tauri command 中直接调用 Win32 API。
...
插件和 AI Chat 只能通过受控接口访问截图、OCR 和翻译结果。插件不得直接访问 platform_win32。
...
共享模型、IPC envelope 和 core_service 调度必须优先写单元测试。Win32、Tauri、egui 的真实集成测试可以后置，但必须保留可替换 trait 或 mock 边界。
...
Phase 1：保持当前骨架可编译，补齐模型、IPC、核心任务队列和规则文档。

Phase 2：接入 Tauri 2 shell、托盘、设置页、全局快捷键开关和本地配置。

Phase 3：接入 egui/eframe/wgpu overlay，完成选区、放大镜、基础 pin window。

Phase 4：接入 Windows 截图 backend，优先 WGC，必要时补 DXGI fallback。

Phase 5：接入 OCR pipeline，先实现任务队列、结果显示和历史，再接远端 provider。

Phase 6：接入翻译 provider、浮动翻译文本和历史。

Phase 7：插件管理、AI Chat、批量历史检索和更多平台适配。
