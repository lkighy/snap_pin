...
pub(crate) fn park_resident_window(
    ctx: &Context,
    window: Option<PlatformWindowRef>,
    ...
    if let Some(window) = window {
        if let Err(error) = platform.window_ops().park_window(
            window,
            Rect::new(
...
pub(crate) fn platform_window_ref_from_raw_window_handle(
    handle: RawWindowHandle,
) -> Option<PlatformWindowRef> {
    match handle {
        RawWindowHandle::Win32(handle) => {
            Some(PlatformWindowRef::from_raw_handle(handle.hwnd.get()))
        }
        _ => None,
    }
}
